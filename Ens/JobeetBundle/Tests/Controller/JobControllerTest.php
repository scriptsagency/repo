...
class JobControllerTest extends WebTestCase
{
  ...
  public function testIndex()
  {
    ...
    $this->assertTrue($crawler->filter('.category_programming tr')->count() == 1);
   ...
   */
    ...
    public function testJobForm()
    {
      ...
      //$this->assertEquals('Ens\JobeetBundle\Controller\JobController::previewAction', $client->getRequest()->attributes->get('_controller'));
        ...
        $query = $em->createQuery('SELECT count(j.id) from EnsJobeetBundle:Job j WHERE j.location = :location AND j.is_public = 1');
        $query->setParameter('location', 'bucharest');
        $this->assertTrue(0 < $query->getSingleScalarResult());
    }
    
    public function testJobForm1()
    {
        $client = static::createClient();
